...
返回值:
******************************************************************************/
bool MsgClient::operator==(const MsgClient &client)
{
...
返回值:
******************************************************************************/
bool MsgClient::register_observer(Message id, CmdTarget &object)
{
	if(id >= MESSAGE_NR)
	{
		return false;
...
返回值:
******************************************************************************/
bool MsgClient::unregister_observer(Message id, const CmdTarget &object)
{
	if(id >= MESSAGE_NR)
	{
		return false;
...
返回值：
	...
	则返回最后一个处理对像的返回值。
******************************************************************************/
int MsgClient::notify_local_observer(const MsgEntity &msg)
{
...
返回值：
******************************************************************************/
void MsgClient::handle_receive_queue(void)
{
	_mutex.lock();
...
返回值：
******************************************************************************/
void MsgClient::handle_send_queue(void)
{
...
返回值：
******************************************************************************/
void MsgClient::_free_send_and_receive_space(void)
{
	MessageEntity *entity = NULL;
...
返回值：
******************************************************************************/
void MsgClient::add_message_to_receive_queue(const MsgEntity &msg)
{
	if(msg.id >= MESSAGE_NR)
	{
		printf("Invalid Message ID: %d\n", msg.id);
...
返回值：
******************************************************************************/
void MsgClient::add_message_to_send_queue(const MsgEntity &msg)
{
	if(msg.id > MESSAGE_NR)
	{
		printf("Invalid Message ID: %d\n", msg.id);
...
返回值：
******************************************************************************/
void MsgClient::clear_message_queue(void)
{
	lock_guard<std::recursive_mutex> lock(_mutex);
...
返回值：
******************************************************************************/
void MsgClient::handle_message(void)
{
...
MsgClient::MsgClient()
{
	...
	// 就有消息传到该线程。MsgClient::add_message_to_receive_queue被异步调用，在此
...
//	assert(_sub_socket);
//
//	int rc = zmq_connect(_sub_socket, MsgCenter::MESSAGE_CENTER_ENDPOINT);
//	assert(rc == 0);
//
...
//	assert(_pub_socket);
//
//	rc = zmq_bind(_pub_socket, MsgCenter::MESSAGE_CENTER_ENDPOINT);
//	assert(rc == 0);

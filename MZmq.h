...
namespace LTUtilities {
	...
	static int RecvAllMessages(void* s,
		...
		std::vector<std::shared_ptr<std::vector<unsigned short>> > & contents,
		int & errNum)
	{
		zmq_msg_t msg;
		int more = 0, size = 0;
		...
		while (1) {
			...
			std::shared_ptr<std::vector<unsigned short>> pcontent(new std::vector<unsigned short>());
	...
	static int zmq_lt_send(void* s, void *data, int dataLen)
	{
		zmq_msg_t msg;
		int rc = -1;

...
class MsgClient
{
	public:
		// 重载==。
		bool operator==(const MsgClient &client);

		// 注册、撤销对一条消息的关注。
		bool register_observer(Message id, CmdTarget &object);
		bool unregister_observer(Message id, const CmdTarget &object);
	...
	private:
		...
		struct MsgMap
		{
			CmdTarget *_subscriber_nr[SUBSCRIBER_NR];
		...
		typedef std::multimap<Message, CmdTarget*> MsgObjectMap;
		typedef std::pair<Message, CmdTarget*> MsgObjectPair;

...
class CmdTarget
{
	public:
		// 重载==。
		bool operator ==(const CmdTarget &object) const;

		// 订阅一条消息。
		bool subscribe(Message id, CmdTarget &object);

		// 订阅一个范围内的消息。
		void subscribe(Message start_id, Message end_id, CmdTarget &object);

		// 撤销对一条消息的订阅。
		bool unsubscribe(Message id, CmdTarget &object);

		// 撤销对一组消息的订阅。
		void unsubscribe(Message start_id, Message end_id, CmdTarget &object);
	...
	protected:
		...
		void set_client(MsgClient &client);
